  ...
  const handleDelete = () => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Eliminar', style: 'destructive', onPress: () => onDelete(budget._id) },
      ]
    );
  };
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <View style={styles.titleContainer}>
          <Text size="md" type="blackText" style={styles.title}>{budget.name}</Text>
          <View style={[styles.statusBadge, { backgroundColor: getStatusColor(budget.estado) }]}>
            <Text size="xs" type="whiteText">{budget.estado}</Text>
          </View>
        </View>
      ...
      <View style={styles.content}>
        <View style={styles.amountContainer}>
          <Text size="lg" type="cbBlueText" style={styles.amount}>{formatCurrency(budget.montoTotal)}</Text>
        </View>
        <View style={styles.dateContainer}>
          <View style={styles.dateItem}>
            <Text size="xs" type="grayText">Inicio</Text>
            <Text size="sm" type="blackText">{formatDate(budget.fechaInicio)}</Text>
          </View>
          <View style={styles.dateSeparator} />
          <View style={styles.dateItem}>
            <Text size="xs" type="grayText">Fin</Text>
            <Text size="sm" type="blackText">{formatDate(budget.fechaFin)}</Text>
          </View>
        </View>
      </View>
    </View>
...
const styles = StyleSheet.create({
  container: { backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 12, shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 3.84, elevation: 5, borderWidth: 1, borderColor: '#f0f0f0' },
  ...
  amount: { fontWeight: 'bold', fontSize: 24 },
  ...
  dateSeparator: { width: 1, height: 30, backgroundColor: '#e0e0e0', marginHorizontal: 16 },
});
